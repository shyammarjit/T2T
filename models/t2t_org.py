...
class Token_performer(nn.Module):
    ...
    def single_attn(self, x):
        print(x[0][0][0:50])
        dim = 147
        kqv = nn.Linear(dim, 3 * self.emb).to('cuda')
...
class T2T_module(nn.Module):
    ...
    def forward(self, x):
        ...
        #shyam
        device = x.device
        x = np.load('/home2/shyammarjit/eff/VTs-Drloc/numpy_array_1.npy')
        x = torch.from_numpy(x).float().to(device)
        #print(x[0][0][0][0:5])

        # temp = x.detach().cpu().numpy()#.numpy()
        # np.save('numpy_array_1.npy', temp)
        ...
        #self.attention1 = Token_transformer(dim=in_chans * 7 * 7, in_dim=token_dim, num_heads=1, mlp_ratio=1.0)
        x = self.attention1(x)
        #print('x_input1', x.shape) # [64, 3136, 64]
        print(torch.any(x.isnan()).item())
        #print(x[0][0][0:5])
        shyam
        ...
        #print('x_input2', x.shape) # [64, 64, 56, 56] 
        print(torch.any(x.isnan()).item())

        # iteration1: soft split
        x = self.soft_split1(x).transpose(1, 2)
        #print('x_input3', x.shape) # [64, 784, 576] 
        print(torch.any(x.isnan()).item())

        # iteration2: re-structurization/reconstruction
        x = self.attention2(x)
        #print('x_input4', x.shape) # [64, 784, 64] 
        print(torch.any(x.isnan()).item())

        B, new_HW, C = x.shape
        x = x.transpose(1, 2).reshape(B, C, int(np.sqrt(new_HW)), int(np.sqrt(new_HW)))
        #print('x_input5', x.shape) # [64, 64, 28, 28]
        print(torch.any(x.isnan()).item()) 

        # iteration2: soft split
        x = self.soft_split2(x).transpose(1, 2)
        #print('x_input6', x.shape) # [64, 196, 576] 
        print(torch.any(x.isnan()).item())

        # final tokens
        x = self.project(x)
        #print('x_input7', x.shape) # [64, 196, 384]
        print(torch.any(x.isnan()).item())
        shyam
        return x
...
class T2T_ViT(nn.Module):
    def __init__(self, 
        ...
        #print(self.tokens_to_token.shape)
        # shyam
