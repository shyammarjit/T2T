...
from models.our import MaskedAutoencoderViT
...
def parse_option():
    ...
    parser.add_argument(
        "--use_drloc", action="store_true", help="Use Dense Relative localization loss"
    )
    parser.add_argument(
        "--drloc_mode", type=str, default="l1", choices=["l1", "ce", "cbr"]
    )
    parser.add_argument(
        "--lambda_drloc",
        type=float,
    ...
    parser.add_argument("--lambda_weight", type = float, default = 0.1, help = "")
    ...
    parser.add_argument(
        "--ape", action="store_true", help="using absolute position embedding"
    )
    parser.add_argument(
        "--rpe", action="store_false", help="using relative position embedding"
    )
...
def main():
    ...
    model = MaskedAutoencoderViT()#build_model(config)
    model.cuda()
    #logger.info(str(model))

    optimizer = build_optimizer(config, model)
    # print(config.AMP_OPT_LEVEL)
    if config.AMP_OPT_LEVEL != "O0": # not running
...
def train_one_epoch(
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        if config.TRAIN.ACCUMULATION_STEPS > 1:
            ...
            loss = criterion_sup(outputs, targets)
            loss = config.lambda_weight * loss + (1 - config.lambda_weight) * loss_twobranch
            # if config.TRAIN.USE_DRLOC:
            #     loss_ssup, ssup_items = criterion_ssup(outputs, config, lambda_drloc)
            #     loss += loss_ssup
            optimizer.zero_grad()
...
def validate(config, data_loader, model, logger):
    ...
    for idx, (images, target) in enumerate(data_loader):
        ...
        # compute output
        loss_twobranch, output = model(images)
